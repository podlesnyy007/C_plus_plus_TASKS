...
double dlina(point a, point b) { //функция, вычисляющая расстояние между двумя точками на плоскости
...
int main()
{
	...
	point a[10]; //описание одномерного массива, где каждый элемент имеет тип point
	int n;
	cout << "Количество точек: ";
	cin >> n;
	for (int i = 0; i < n; i++) {
		cout << "Координаты точки " << i + 1 << ": ";
		cin >> a[i].x >> a[i].y; //через точку идет доступ к полю структуры
	}
	double k = 0;
	for (int i = 0; i < n; i++) { //ищем наибольшую сумму расстояний каждой точки
		double sum = 0;
		for (int j = 0; j < n; j++)
			sum += dlina(a[i], a[j]);
		if (k < sum)
